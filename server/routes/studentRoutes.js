...
router.put('/student/profile', verifyToken, authorize('student'), async (req, res) => {
  try {
...
router.get('/attendance-sessions/active', verifyToken, authorize('student'), async (req, res) => {
  try {
    ...
    const sessionsWithStatus = sessions.map(s => {
      const alreadyMarked = s.attendances.some(a => a.email === studentEmail);
      return {
        _id: s._id,
        course: s.courseId,
        location: s.location,
...
// POST /api/attendance/mark — submit QR attendance with full validation
router.post('/attendance/mark', verifyToken, authorize('student'), async (req, res) => {
  try {
    const { sessionId, qrToken, lat, lng } = req.body;
    const studentEmail = req.user.email;

    if (!sessionId || !qrToken || lat === undefined || lng === undefined) {
      return res.status(400).json({ message: 'sessionId, qrToken, lat, and lng are required' });
    }
...
router.get('/attendance-sessions/history', verifyToken, authorize('student'), async (req, res) => {
  try {
    const studentEmail = req.user.email;
