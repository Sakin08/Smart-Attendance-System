...
router.post('/courses', verifyToken, authorize('teacher'), async (req, res) => {
  try {
    ...
    if (!department || !season || !courseName || !courseCode) {
      return res.status(400).json({ message: 'Department, season, course name, and course code are required' });
    }
...
router.get('/courses', verifyToken, authorize('teacher', 'admin'), async (req, res) => {
  try {
    let filter = {};
...
router.get('/courses/:id', verifyToken, authorize('teacher', 'admin'), async (req, res) => {
  try {
...
router.put('/courses/:id/students', verifyToken, authorize('teacher'), async (req, res) => {
  try {
...
router.post('/attendance-sessions', verifyToken, authorize('teacher'), async (req, res) => {
  try {
    const { courseId, startTime, endTime, lat, lng, radiusMeters } = req.body;

    if (!courseId || !startTime || !endTime || lat === undefined || lng === undefined) {
      return res.status(400).json({ message: 'courseId, startTime, endTime, lat, and lng are required' });
    }
...
router.get('/attendance-sessions/teacher', verifyToken, authorize('teacher'), async (req, res) => {
  try {
...
router.get('/attendance-sessions/:id', verifyToken, authorize('teacher', 'admin'), async (req, res) => {
  try {
    const session = await AttendanceSession.findById(req.params.id)
      .populate('courseId', 'courseName courseCode department season students');
...
router.get('/attendance-sessions/:id/export', verifyToken, authorize('teacher', 'admin'), async (req, res) => {
  try {
    ...
    const enrolledStudents = session.courseId.students || [];
    const data = enrolledStudents.map((email, idx) => {
      const entry = session.attendances.find(a => a.email === email);
      return {
        'SL': idx + 1,
        'Email': email,
        ...
        'Latitude': entry ? entry.lat : '-',
        'Longitude': entry ? entry.lng : '-'
      };
    });
    ...
    if (format === 'excel') {
      ...
      res.setHeader('Content-Disposition', `attachment; filename=attendance_${session._id}.csv`);
      res.send(csv);
    }
  } catch (error) {
...
router.get('/courses/:id/attendance-report', verifyToken, authorize('teacher', 'admin'), async (req, res) => {
  try {
    ...
    res.json({
      ...
      sessions: sessions.map(s => ({ _id: s._id, startTime: s.startTime, endTime: s.endTime })),
      report
    });
  } catch (error) {
